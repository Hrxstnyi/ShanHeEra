...
void UHistorySystem::LoadEasterEggEvents()
{
    ...
    Event1.Title = NSLOCTEXT("Egg","YueFei","精忠报国");
    Event1.Description = NSLOCTEXT("Egg","YueFeiDesc","还我河山的誓言回荡千年，英雄的血脉从未断绝。当玩家在边疆抵御外侮时，可触发此彩蛋。");
    ...
    Event2.Title = NSLOCTEXT("Egg","WenTianXiang","正气歌");
    Event2.Description = NSLOCTEXT("Egg","WenTianXiangDesc","人生自古谁无死，留取丹心照汗青。当玩家面临生死抉择时，可触发此彩蛋。");
    Event2.bIsCG = true;
    HistoricalEvents.Add(Event2);

    FHistoricalEvent Event3;
    Event3.EventID = TEXT("Egg_ZhengHe");
    Event3.Title = NSLOCTEXT("Egg","ZhengHe","郑和下西洋");
    Event3.Description = NSLOCTEXT("Egg","ZhengHeDesc","宝船远航，播撒文明而非征服。当玩家发展航海贸易时，可触发此彩蛋。");
    Event3.bIsCG = true;
    HistoricalEvents.Add(Event3);

    FHistoricalEvent Event4;
    Event4.EventID = TEXT("Egg_ResistanceWar");
    Event4.Title = NSLOCTEXT("Egg","ResistanceWar","众志成城");
    Event4.Description = NSLOCTEXT("Egg","ResistanceWarDesc","地无分南北，人无分老幼，无论何人，皆有守土抗战之责任。近代历史暗喻彩蛋。");
    Event4.bIsCG = true;
    HistoricalEvents.Add(Event4);

    FHistoricalEvent Event5;
    Event5.EventID = TEXT("Egg_TwoBombsOneStar");
    Event5.Title = NSLOCTEXT("Egg","TwoBombs","两弹一星");
    Event5.Description = NSLOCTEXT("Egg","TwoBombsDesc","隐姓埋名于戈壁，以身许国铸重器。当玩家推动技术突破时，可触发此彩蛋。");
    Event5.bIsCG = true;
    HistoricalEvents.Add(Event5);

    FHistoricalEvent Event6;
    Event6.EventID = TEXT("Egg_FloodRelief");
    Event6.Title = NSLOCTEXT("Egg","FloodRelief","人民团结");
    Event6.Description = NSLOCTEXT("Egg","FloodReliefDesc","一方有难，八方支援。当玩家组织救灾时，可触发此彩蛋，展现人民团结的力量。");
    Event6.bIsCG = true;
...
void UHistorySystem::LoadTaiwanEvents()
{
    ...
    Taiwan1.Title = NSLOCTEXT("Taiwan","Annexation","宝岛归心");
    Taiwan1.Description = NSLOCTEXT("Taiwan","AnnexationDesc","台湾自古即为华夏疆域，海峡两岸同文同种。当玩家推动台湾府建设时，触发此剧情，展现两岸血脉相连。");
    Taiwan1.bIsCG = true;
    HistoricalEvents.Add(Taiwan1);

    FHistoricalEvent Taiwan2;
    Taiwan2.EventID = TEXT("Taiwan_Defense");
    Taiwan2.Title = NSLOCTEXT("Taiwan","Defense","守土有责");
    Taiwan2.Description = NSLOCTEXT("Taiwan","DefenseDesc","外敌觊觎宝岛，军民同心御侮。台湾的命运，从来与大陆紧密相连，不可分割。");
    Taiwan2.bIsCG = true;
    HistoricalEvents.Add(Taiwan2);

    FHistoricalEvent Taiwan3;
    Taiwan3.EventID = TEXT("Taiwan_Reunification");
    Taiwan3.Title = NSLOCTEXT("Taiwan","Reunification","九州一统");
    Taiwan3.Description = NSLOCTEXT("Taiwan","ReunificationDesc","海峡浅浅，血脉浓浓。天下一统，民心所向。此剧情暗喻台湾回归是历史必然。");
    Taiwan3.bIsCG = true;
