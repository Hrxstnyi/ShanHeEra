...
void UTutorialSystem::InitializeTutorials()
{
    ...
    Ch1.ChapterName = FText::FromString(TEXT("基础操作"));
    Ch1.bIsUnlocked = true;

    FTutorialStep S1;
    S1.StepID = TEXT("Move"); S1.Title = FText::FromString(TEXT("移动"));
    S1.Description = FText::FromString(TEXT("使用WASD移动角色"));
    S1.Hint = FText::FromString(TEXT("按 W 键向前移动"));
    Ch1.Steps.Add(S1);

    FTutorialStep S2;
    S2.StepID = TEXT("Interact"); S2.Title = FText::FromString(TEXT("交互"));
    S2.Description = FText::FromString(TEXT("靠近NPC或物品按E交互"));
    S2.Hint = FText::FromString(TEXT("按 E 键与附近的人对话"));
    Ch1.Steps.Add(S2);
    ...
    Ch2.ChapterName = FText::FromString(TEXT("人生模式"));
    FTutorialStep S3;
    S3.StepID = TEXT("OpenMenu"); S3.Title = FText::FromString(TEXT("打开菜单"));
    S3.Description = FText::FromString(TEXT("按Tab打开角色菜单查看属性"));
    Ch2.Steps.Add(S3);
    Chapters.Add(Ch2);
