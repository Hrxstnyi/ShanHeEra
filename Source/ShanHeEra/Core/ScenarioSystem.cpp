...
void UScenarioSystem::InitializeScenarios()
{
    ...
    ThreeK.ScenarioName = NSLOCTEXT("Scenario","ThreeK","三国群雄");
    ThreeK.Description = NSLOCTEXT("Scenario","ThreeKDesc","东汉末年，天下大乱，群雄并起。你将在这个英雄辈出的时代中寻找自己的位置。");
    ThreeK.StartYear = 184; ThreeK.StartMonth = 1;
    ThreeK.BackgroundStory = NSLOCTEXT("Scenario","ThreeKStory","黄巾起义爆发，汉室衰微。各路诸侯拥兵自重，天下三分的大势正在形成。");
    ...
    Tang.ScenarioName = NSLOCTEXT("Scenario","Tang","盛唐气象");
    Tang.Description = NSLOCTEXT("Scenario","TangDesc","贞观之治，万国来朝。这是中华文明最辉煌的时代，你将见证大唐的盛世荣光。");
    Tang.StartYear = 627; Tang.StartMonth = 1;
    Tang.BackgroundStory = NSLOCTEXT("Scenario","TangStory","李世民登基，改元贞观。大唐国力蒸蒸日上，丝绸之路畅通无阻，文化艺术空前繁荣。");
    ...
    Song.ScenarioName = NSLOCTEXT("Scenario","Song","弱宋偏安");
    Song.Description = NSLOCTEXT("Scenario","SongDesc","靖康之耻后，宋室南渡。外有强敌环伺，内有党争不断，你能否为大宋找到一条出路？");
    Song.StartYear = 1127; Song.StartMonth = 1;
    Song.BackgroundStory = NSLOCTEXT("Scenario","SongStory","金兵南下，徽钦二帝被掳。赵构在临安即位，建立南宋。岳飞等名将正在为收复中原而战。");
    ...
    Ming.ScenarioName = NSLOCTEXT("Scenario","Ming","明末风云");
    Ming.Description = NSLOCTEXT("Scenario","MingDesc","内有流寇作乱，外有后金虎视。崇祯皇帝有心无力，大明王朝走到了命运的十字路口。");
    Ming.StartYear = 1628; Ming.StartMonth = 1;
    Ming.BackgroundStory = NSLOCTEXT("Scenario","MingStory","崇祯登基，铲除魏忠贤。但天下已经千疮百孔——陕西大旱，李自成起义，努尔哈赤的后金在辽东崛起。");
